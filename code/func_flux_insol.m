...
%% Vectorize fields (2D to 1D) that are required by other functions
% insol.TOA = reshape(insol.TOA,[grid.Lx*grid.Ly,1]);
% insol.TOA = insol.TOA(grid.mask>0);
% insol.TOAflat = reshape(insol.TOAflat,[grid.Lx*grid.Ly,1]);
% insol.TOAflat = insol.TOAflat(grid.mask>0);
